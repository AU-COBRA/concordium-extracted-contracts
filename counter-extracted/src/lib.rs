...
fn counter_init(&'a self, chain: &'a Chain<'a>, ctx: &'a ContractCallContext<'a>, init_value: i64) -> Option<&'a State<'a>> {
  Some(
    self.alloc(
      State::build_state(
        PhantomData,
...
fn counter_init__curried(&'a self) -> &'a dyn Fn(&'a Chain<'a>) -> &'a dyn Fn(&'a ContractCallContext<'a>) -> &'a dyn Fn(i64) -> Option<&'a State<'a>> {
...
fn counter(&'a self, st: &'a State<'a>, msg: &'a Msg<'a>) -> Option<&'a State<'a>> {
  match msg {
    &Msg::Inc(_, i) => {
      match self.ltb(
              0,
...
fn counter__curried(&'a self) -> &'a dyn Fn(&'a State<'a>) -> &'a dyn Fn(&'a Msg<'a>) -> Option<&'a State<'a>> {
  self.closure(move |st| {
...
fn counter_receive(&'a self, chain: &'a Chain<'a>, ctx: &'a ContractCallContext<'a>, state: &'a State<'a>, msg: Option<&'a Msg<'a>>) -> Option<__pair<&'a State<'a>, &'a List<'a, ActionBody<'a>>>> {
  match msg {
    Some(m) => {
      match self.counter(
              state,
...
fn counter_receive__curried(&'a self) -> &'a dyn Fn(&'a Chain<'a>) -> &'a dyn Fn(&'a ContractCallContext<'a>) -> &'a dyn Fn(&'a State<'a>) -> &'a dyn Fn(Option<&'a Msg<'a>>) -> Option<__pair<&'a State<'a>, &'a List<'a, ActionBody<'a>>>> {
  self.closure(move |chain| {
    self.closure(move |ctx| {
